...
class MainGameLoop implements Runnable {
    ...
    private void executeRequests() {
        ...
        int i;
        for (i = 0; i < thread.vaoCreateRequests.size(); i++)
            thread.vaoCreateRequests.get(i).execute();
        for (i = 0; i < thread.toCarryOutRequests.size(); i++)
            thread.toCarryOutRequests.get(i).execute();
        for (i = 0; i < thread.renderingRequests.size(); i++)
    ...
    public void run() {
        ...
        bluredSceneFBO = new FBO(GU.WIDTH / 4, GU.HEIGHT / 4, FBO.COLOR_TEXTURE).create();
